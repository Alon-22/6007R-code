...
void autonomous() {
  ...
  drive(0, 0, 0, 0, 700);
  //above is safe 1 pt auto, lower is to grab some cubes and maybe score them
  clawClose(50,700);
  liftUp(20,100);
  liftDown(20,100);
  drive(75, 75, -75, -75, 800);
  drive(0, 0, 0, 0, 1000);
  drive(50, 50, 50, 50, 1000);
  drive(0, 0, 0, 0, 50);
  clawClose(50,1000);
  liftUp(60,800);
  drive(-75, -75, 75, 75, 800);
...
void userControl() {
  ...
  while (true) {
    ...
    if (Controller.ButtonY.pressing()) {
     fourbar.spinTo(2, turns, 0);
    } else if (Controller.ButtonB.pressing()) {
      fourbar.spinTo(-2, turns, 0);
    }
    else if (Controller.ButtonY.pressing()&&Controller.ButtonUp.pressing()) {
     fourbar.spin(fwd, 100, rpm);
    }
    else if (Controller.ButtonB.pressing()&&Controller.ButtonUp.pressing()) {
     fourbar.spin(fwd, -100, rpm);
    }
    else{
      fourbar.stop(hold);
